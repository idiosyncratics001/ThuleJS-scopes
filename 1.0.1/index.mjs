    ...
    fs.readdirSync(dir).forEach((file) => {
      ...
      if (stat.isDirectory()) {
        filePaths[file]= getFilePaths(path)
          ...
          if (!file.slice(-2) === 'js' || !file.slice(-3) === 'mjs') return
...
function getFileContent(obj, reloadScopeName) {
    ...
    function importScope (filePath, scopeName) {
        ...
        try {
          var getFile = fs.readFileSync(filePath, 'utf8')
          data = getFile;
        }catch(error){
...
export default class _scopes {
    ...
    static getScopes(...scopeName){
      ...
      if(scopeName.length == 1){
        ...
        if(fs.existsSync(returnScope)) returnScope = getFileContent({returnScope},scopeName[0]).returnScope
        if(!returnScope){
    ...
    static buildScopes(...scopeName){
        ...
        if (scopeName.length == 1){
            ...
            if (this.#meta.preload || scopePath.cached || scopePath.path){   
